...
def query_all_grads():
    ...
    """
    db.query_all_from_table('graduates')

def __prepare_argument(user_input):
    prepped_arg = '%' + user_input + '%'
    return prepped_arg

def search_grads(name='', dept='', bio='', experience='', industry='', interest=''):
    """
    ...
    command = sqla.text('SELECT DISTINCT graduates.id, graduates.name FROM graduates, grad_experiences, grad_industries, grad_interests '\
    ...
    params = {'name': name, 'dept': dept, 'bio': bio, 'experience': experience, 'industry': industry, 'interest': interest}
...
def get_grad_information(idnum):
    ...
    """
    command = sqla.text('SELECT * FROM graduates WHERE graduates.id = :id')
    params = {'id': idnum}
    output = db.execute_command(command, params)
    return [x for x in output][0]

def add_a_grad(name=None, dept=None, bio=None, photo_link=None, website_link=None, experiences=None, industries=None, interests=None, email=None, phone=None):
    """
    ...
    if experiences is not None:
        for experience, desc in experiences:
            experience_info = {'id': new_id, 'experience': experience, 'experience_desc': desc}
            statement = sqla.text("""INSERT INTO grad_experiences(id, experience, experience_desc) VALUES(:id, :experience, :experience_desc)""")
            output = db.execute_command(statement, experience_info)

    if industries is not None:
        for industry in industries:
            industry_info = {'id': new_id, 'industry': industry}
            statement = sqla.text("""INSERT INTO grad_industries(id, industry) VALUES(:id, :industry)""")
            output = db.execute_command(statement, industry_info)

    if interests is not None:
        for interest in interests:
            interest_info = {'id': new_id, 'interest': interest}
            statement = sqla.text("""INSERT INTO grad_interests(id, interest) VALUES(:id, :interest)""")
            output = db.execute_command(statement, interest_info)

    if email is not None or phone is not None:
        contact_info = {'id': new_id, 'email': email, 'phone': phone}
        statement = sqla.text("""INSERT INTO grad_contact(id, email, phone) VALUES(:id, :email, :phone)""")
...
def del_a_grad(del_id):
    ...
    db.del_id_from_tables(['grad_contact', 'grad_experiences', 'grad_industries', 'grad_interests', 'graduates'], del_id)
